...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
  return (
    <thead
      ...
      className={cn("bg-muted/40 text-xs uppercase tracking-[0.2em] [&_tr]:border-b", className)}
      {...props}
    />
  )
}
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "text-foreground/80 h-11 px-3 text-left align-middle font-semibold whitespace-nowrap [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]",
        className
      )}
      {...props}
    />
